...
class FileCache {
   ...
   */
  async initialize() {
    this.database = await MainCache.getFilesDatabaseInstance(this.logger, this.cacheFolder)
    this.filesErrorDatabase = await MainCache.getFilesErrorDatabaseInstance(this.logger, this.cacheFolder)
  ...
  async cacheFile(cachePath, timestamp) {
    ...
    await databaseService.saveFile(this.database, timestamp, this.api.application.id, cachePath)
  ...
  async sendFile() {
    ...
    try {
      const fileToSend = await databaseService.getFileToSend(this.database, id)
      ...
      try {
        ...
        await databaseService.deleteSentFile(this.database, id, fileToSend.path)
  ...
  async handleSentFile(filePath) {
    ...
    const count = await databaseService.getFileCount(this.database, filePath)
    if (count === 0) {
      if (this.archiveMode) {
  ...
  async getStats(apiName) {
    const cacheSize = await databaseService.getFileCountForApi(this.database, apiName)
    return {
