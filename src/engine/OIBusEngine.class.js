...
class OIBusEngine extends BaseEngine {
  ...
  async getStatus() {
    ...
    const filesErrorDatabase = await MainCache.getValuesErrorDatabaseInstance(this.logger, this.getCacheFolder())
    const filesErrorCount = await databaseService.getErroredFilesCount(filesErrorDatabase)
    const valuesErrorDatabase = await MainCache.getValuesErrorDatabaseInstance(this.logger, this.getCacheFolder())
    const valuesErrorCount = await databaseService.getErroredValuesCount(valuesErrorDatabase)
